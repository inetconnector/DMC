...
int main(int argc, char ** argv) {
    ...
    const std::size_t queries = argc > 4 ? std::strtoull(argv[4], nullptr, 10) : 32;
    const std::size_t repeats = argc > 5 ? std::strtoull(argv[5], nullptr, 10) : 10;
    ...
    cfg.retrieved_blocks = 32;
    cfg.route_refresh = 8;

    std::mt19937 rng(23);
    auto refs = std::vector<TokenRef>();
    refs.reserve(tokens);
    for (std::size_t i = 0; i < tokens; ++i) {
        refs.push_back(TokenRef{static_cast<std::uint32_t>(i), static_cast<pos_t>(i), 0});
    }

    auto route_keys = make_random(tokens * route_dim, rng);
    ...
    const auto [select_ms, route] = measure([&] {
        return idx.route(0, static_cast<pos_t>(tokens - 1), route_query, 8);
    }, repeats);

    if (route.cells.empty()) {
        std::cerr << "Bridge route returned no cells\n";
        return 1;
    }
    ...
    const auto [cpu_ms, cpu_out] = measure([&] {
        return exact_selected_attention(q, k, v, route.cells, head_dim).output;
    }, repeats);

    const auto [gpu_total_ms, gpu_out] = measure([&] {
        return run_selected_attention_cuda(q, k, v, route.cells, head_dim);
    }, repeats);

    const auto gpu_kernel_ms = gpu_out.kernel_ms;
    const auto gpu_cpu_diff = max_abs_diff(cpu_out, gpu_out.output);
    const auto dense_gpu_diff = max_abs_diff(dense_out, run_selected_attention_cuda(q, k, v, all_ids, head_dim).output);
    ...
    std::cout << std::fixed << std::setprecision(3)
              ...
              << "  \"selected_cells\": " << route.cells.size() << ",\n"
              << "  \"selected_spans\": " << route.spans.size() << ",\n"
              << "  \"dense_ms\": " << dense_ms * 1000.0 << ",\n"
              << "  \"cpu_selected_ms\": " << cpu_ms * 1000.0 << ",\n"
              << "  \"gpu_total_ms\": " << gpu_total_ms * 1000.0 << ",\n"
              << "  \"gpu_kernel_ms\": " << gpu_kernel_ms << ",\n"
              << "  \"cpu_vs_gpu_max_abs_diff\": " << gpu_cpu_diff << ",\n"
              << "  \"dense_vs_gpu_full_max_abs_diff\": " << dense_gpu_diff << "\n"
              << "}\n";
}
